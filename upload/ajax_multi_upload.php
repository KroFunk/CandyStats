...
$skipEventVariableArray = array("waskilledbythebomb.", "STEAM USERID validated","entered the game","switched from team <Unassigned> to <TERRORIST>","switched from team <Unassigned> to <CT>","switched from team <TERRORIST> to <CT>","switched from team <CT> to <TERRORIST>","switched from team <CT> to <Unassigned>","switched from team <TERRORIST> to <Unassigned>");
$skipMiscArray  = array("Planted_The_Bomb","Dropped_The_Bomb","Got_The_Bomb","Player has left the game - IGNORE","Rescued_A_Hostage","committed suicide","Touched_A_Hostage","connected","connected, address ","purchased","disconnected","disconnected (reason ","switched from team <Unassigned> to <TERRORIST>","switched from team <Unassigned> to <CT>","switched from team <TERRORIST> to <CT>","switched from team <CT> to <TERRORIST>","switched from team <CT> to <Unassigned>","switched from team <TERRORIST> to <Unassigned>", "Begin_Bomb_Defuse_Without_Kit");//Sometimes spaces are important!
...
function PlayerActive() {
    if(in_array($GLOBALS['SteamID'],$GLOBALS['activePlayers'])){
        ...
        if(!in_array($GLOBALS['SteamID'],$GLOBALS['inactivePlayers'])){
            ...
            //echo "<span style='color:#8e7bd5'>[CandyStats]</span> <span style='color:#ccac30;'>Query: " . $GLOBALS['$queryString'] . "</span>" . PHP_EOL;
...
if($debug==true){
    ...
    while ($data = fgets($handle,4096)) { 
        ...
        if(stripos($string,'Loading map') !== false){
            ...
            echo "<span style='color:#8e7bd5'>[CandyStats]</span> MapInfo: ". $EventVariable . "..." . PHP_EOL;
            $MapInfo = $EventVariable;
            ...
            //Enter World Row into MySQL!
            echo "<span style='color:#8e7bd5'>[CandyStats]</span> Prepping MySQL Command..." . PHP_EOL;
            $queryString .= "INSERT INTO `logdata` (`CSID`, `SessionID`, `TIMESTAMP`, `TAGS`, `Name`, `SteamID`, `Team`, `EventType`, `EventVariable`, `Misc_1`, `Misc_3`, `XYZ_1`, `MapInfo`) VALUES (NULL, '" . htmlentities($SessionID, ENT_QUOTES) . "', '" . htmlentities($TIMESTAMP, ENT_QUOTES) . "', '', '" . htmlentities($Name, ENT_QUOTES) . "', '" . htmlentities($SteamID, ENT_QUOTES) . "', NULL, '" . htmlentities($EventType, ENT_QUOTES) . "', '" . htmlentities($EventVariable, ENT_QUOTES) . "', NULL, '" . htmlentities($Misc_3, ENT_QUOTES) . "', NULL, '" . htmlentities($MapInfo, ENT_QUOTES) . "');";
            $rowAccept = False;
            //echo "<span style='color:#8e7bd5'>[CandyStats]</span> <span style='color:#ccac30;'>Query: " . $queryString . "</span>" . PHP_EOL;
            //mysqli_query($con, $queryString) or die("There was a problem with the query and the script has been stopped." . mysqli_error($con));
