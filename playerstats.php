<?php
  ...
  //Most purchased item
  $queryString = "SELECT `FriendlyName`, count(*) as `MostPurchased` FROM `logdata` INNER JOIN `itemdetails` ON `logdata`.`EventVariable` = `itemdetails`.`Weapon` WHERE `EventType` = 'purchased' AND `SteamID` LIKE '" . $HTMLSteamID . "' GROUP BY `EventVariable` ORDER BY `MostPurchased` DESC LIMIT 1";
  ...
  $queryString = "SELECT `FriendlyName`, count(*) as `LeastPurchased` FROM `logdata` INNER JOIN `itemdetails` ON `logdata`.`EventVariable` = `itemdetails`.`Weapon` WHERE `EventType` = 'purchased' AND `SteamID` LIKE '" . $HTMLSteamID . "' GROUP BY `EventVariable` ORDER BY `LeastPurchased` ASC LIMIT 1";
